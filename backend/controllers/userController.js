...
export const getUserById = async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `SELECT id, name, first_name, last_name, email, role, avatar_url,
                    phone, country, city, state, zip_code, address, is_verified, is_active
             FROM users WHERE id=$1`,
            [id]
        );
        if (result.rows.length === 0) return res.status(404).json({ error: "User not found" });
...
export const updateUserById = async (req, res) => {
    try {
        ...
        const {
            ...
            phone, country, city, state, zip_code, address, is_active
        } = req.body;
        ...
        await pool.query(
            "INSERT INTO audit_logs (user_id, action, ip_address) VALUES ($1,$2,$3)",
            [req.user.id, `ADMIN_UPDATE_USER: ${result.rows[0].email}`, req.ip]
        );
